...
ticker = st.selectbox(
    ...
    format_func=lambda value: (
        f"{filtered.loc[filtered.id == value, 'company_name'].iloc[0]} ({value})"
    ),
)
...
st.write(
    company.about_company or "Company description is not available in the source data."
)
...
kpis(
    {
        ...
        "Net margin": value("net_margin_pct") + "%",
        "D/E": value("debt_to_equity"),
        "Revenue CAGR 5yr": value("cagr_5y") + "%",
        "FCF": value("free_cash_flow"),
    }
)
if pl.empty:
    st.info("Data available note: no profit and loss history was loaded.")
    st.stop()
fig = go.Figure()
...
if not ratios.empty:
    fig = go.Figure()
    fig.add_trace(go.Scatter(x=ratios.year, y=ratios.return_on_equity_pct, name="ROE"))
    fig.add_trace(
        go.Scatter(
            x=ratios.year,
