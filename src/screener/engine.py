...
def latest_ratios(ratios: pd.DataFrame) -> pd.DataFrame:
    return ratios.sort_values(["company_id", "year"]).drop_duplicates("company_id", keep="last").copy()
...
def composite_quality_score(ratios: pd.DataFrame) -> pd.DataFrame:
    frame = latest_ratios(ratios)
    metrics = ["return_on_equity_pct", "return_on_capital_employed_pct", "net_margin_pct", "interest_coverage", "cash_conversion", "cagr_5y", "debt_to_equity"]
    scores = []
    for metric in metrics:
        values = pd.to_numeric(frame[metric], errors="coerce"); lo, hi = values.quantile(.10), values.quantile(.90); clipped = values.clip(lo, hi)
        percentile = clipped.rank(pct=True) * 100; scores.append(100 - percentile if metric == "debt_to_equity" else percentile)
...
def generate_screener_outputs(ratios: pd.DataFrame, companies: pd.DataFrame) -> None:
    ...
    merged = ratios.merge(companies[["id", "company_name"]], left_on="company_id", right_on="id", how="left")
    ...
    with pd.ExcelWriter(output / "screener_output.xlsx", engine="openpyxl") as writer:
        for name, thresholds in PRESETS["presets"].items():
            apply_screen(merged, thresholds).to_excel(writer, sheet_name=name[:31], index=False)
        for sheet in writer.book.worksheets:
            if sheet.max_row > 1 and sheet.max_column > 1:
                sheet.freeze_panes = "A2"
                sheet.auto_filter.ref = sheet.dimensions
                sheet.conditional_formatting.add(f"B2:{sheet.cell(sheet.max_row, sheet.max_column).coordinate}", ColorScaleRule(start_type="min", start_color="F8696B", mid_type="percentile", mid_value=50, mid_color="FFEB84", end_type="max", end_color="63BE7B"))
    percentiles = build_peer_percentiles(merged, companies); write_peer_outputs(percentiles)
    return score