        ...
        rows.append(
            {
                ...
                "company_name": None
                if company.empty
                else company.iloc[0]["company_name"],
                "child_rows": sum(child_counts.values()),
                "child_row_counts": "; ".join(
                    f"{name}={count}" for name, count in child_counts.items()
                ),
...
def load_database(
    ...
    with sqlite3.connect(database_path) as connection:
        ...
        audit_rows.append(
            {
                ...
                "source_rows": len(ratios),
                "rows_loaded": len(ratios),
                "rows_rejected": 0,
            }
        )
        ...
        peer_frame = percentiles.reindex(columns=peer_columns).where(
            percentiles.notna(), None
        )
        connection.executemany(
        ...
        audit_rows.append(
            {
                "dataset": "peer_percentiles",
                "source_rows": len(percentiles),
                "rows_loaded": len(percentiles),
                "rows_rejected": 0,
            }
        )
    ...
    latest_patterns = ratios.sort_values(
        ["company_id", "year"], na_position="first"
    ).drop_duplicates("company_id", keep="last")
    latest_patterns[["company_id", "capital_allocation_pattern"]].to_csv(
...
def main() -> None:
    ...
    print(
        f"Manual review: {int((review['status'] == 'PASS').sum())}/{len(review)} passed"
    )
