...
def parse_analysis(analysis: pd.DataFrame) -> tuple[pd.DataFrame, pd.DataFrame]:
    ...
    for _, row in analysis.iterrows():
        for field in FIELDS:
            ...
            if match:
                parsed.append({"company_id": row.company_id, "metric_type": field, "period_years": int(match.group(1)), "value_pct": float(match.group(2))})
            else:
                failures.append({"company_id": row.company_id, "metric_type": field, "raw_text": text, "reason": "pattern_not_matched"})
    return pd.DataFrame(parsed, columns=["company_id", "metric_type", "period_years", "value_pct"]), pd.DataFrame(failures, columns=["company_id", "metric_type", "raw_text", "reason"])


def cross_validate(parsed: pd.DataFrame, datasets: dict[str, pd.DataFrame]) -> pd.DataFrame:
    ...
    for _, row in parsed.iterrows():
        ...
        if computed_column is None: continue
        computed = latest.loc[latest.company_id == row.company_id, computed_column]
        if computed.empty or pd.isna(computed.iloc[0]) or row.period_years != 5: continue
        divergence = abs(float(row.value_pct) - float(computed.iloc[0]))
        if divergence > 5:
            checks.append({"company_id": row.company_id, "metric_type": row.metric_type, "period_years": row.period_years, "source_value_pct": row.value_pct, "computed_value_pct": computed.iloc[0], "divergence_pct": divergence, "manual_review": True})
    return pd.DataFrame(checks)


def write_outputs(output_dir: Path = ROOT / "output") -> tuple[pd.DataFrame, pd.DataFrame]:
    ...
    cross_validate(parsed, datasets).to_csv(output_dir / "analysis_cross_validation.csv", index=False)
