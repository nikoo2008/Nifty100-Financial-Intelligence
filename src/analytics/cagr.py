...
def calculate_cagr(
    start_value: Any, end_value: Any, years: float
...
def cagr_for_periods(
    ...
    if history.empty or "year" not in history or value_column not in history:
        return {f"cagr_{period}y": None for period in periods} | {
            "cagr_edge_case": "insufficient_history"
        }
    ...
    for period in periods:
        start = (
            frame[frame["year"] == latest_year - period][value_column]
            if latest_year is not None
            else pd.Series(dtype=float)
        )
        value, reason = calculate_cagr(
            None if start.empty else start.iloc[-1], latest, period
        )
        result[f"cagr_{period}y"] = value
        if reason and result["cagr_edge_case"] is None:
            result["cagr_edge_case"] = reason
    if (
        all(result[f"cagr_{period}y"] is None for period in periods)
        and result["cagr_edge_case"] is None
    ):
        result["cagr_edge_case"] = "insufficient_history"
    return result


__all__ = ["EDGE_CASE_FLAGS", "cagr_for_periods", "calculate_cagr"]
