...
def calculate_cashflow_kpis(
    pnl: pd.Series, cashflow: pd.Series | None = None
    ...
    free_cash_flow = (
        None if operating is None or investing is None else operating + investing
    )
    ...
    return {
        ...
        "operating_cash_flow_margin_pct": None
        if ocf_margin is None
        else ocf_margin * 100.0,
...
def build_cashflow_intelligence(
    ...
    for company_id in companies["id"].astype(str):
        ...
        cf = (
            cashflow[cashflow.company_id.astype(str) == company_id]
            .copy()
            .sort_values("year")
        )
        bs = (
            balancesheet[balancesheet.company_id.astype(str) == company_id]
            .copy()
            .sort_values("year")
        )
        ...
        merged["cfo_pat"] = merged.operating_activity / merged.net_profit.replace(
            0, pd.NA
        )
        ...
        if not latest.empty and pd.notna(latest_row.sales) and latest_row.sales:
            capex = (
                abs(float(latest_row.investing_activity or 0))
                / float(latest_row.sales)
                * 100
            )
        ...
        if len(fcf.dropna()) >= 2:
            ...
            years = max(
                int(merged.year.dropna().iloc[-1] - merged.year.dropna().iloc[0]), 1
            )
            if start > 0 and end > 0:
                fcf_cagr = ((end / start) ** (1 / years) - 1) * 100
        fcf_conversion = (
            None
            if latest.empty or not latest_row.net_profit
            else float(
                (latest_row.operating_activity + latest_row.investing_activity)
        ...
        distress = bool(
            not latest.empty
            and latest_row.operating_activity < 0
            and latest_row.financing_activity > 0
        )
        debt = pd.to_numeric(bs.borrowings, errors="coerce")
        deleveraging = bool(
            len(debt.dropna()) >= 2
            and debt.dropna().iloc[-1] < debt.dropna().iloc[-2]
        ...
        sector = (
            ...
            if any(
                word
                in f"{company_id} {companies.loc[companies.id.astype(str) == company_id, 'company_name'].iloc[0]}".lower()
        ...
        row = {
            ...
            "cfo_quality_label": _label_cfo_quality(score),
            "capex_intensity_pct": capex,
            "capex_label": _label_capex(capex),
        ...
        if distress:
            distress_rows.append(
                {
                    "company_id": company_id,
                    "cfo_value": latest_row.operating_activity,
                    "cff_value": latest_row.financing_activity,
                    "latest_net_profit": latest_row.net_profit,
                }
            )
    return pd.DataFrame(rows), pd.DataFrame(distress_rows)


def write_cashflow_outputs(
    database_path: Path | str = Path("nifty100.db"),
    ...
    with sqlite3.connect(database_path) as connection:
        tables = {
            ...
            for name in ["companies", "profitandloss", "cashflow", "balancesheet"]
        }
        ratio_frame = pd.read_sql_query("SELECT * FROM financial_ratios", connection)
    result, alerts = build_cashflow_intelligence(
        tables["profitandloss"],
        tables["cashflow"],
        tables["balancesheet"],
        tables["companies"],
    )
    pattern = ratio_frame.sort_values("year").drop_duplicates(
        "company_id", keep="last"
    )[["company_id", "capital_allocation_pattern"]]
    result = (
        result.drop(columns=["capital_allocation_label"])
        .merge(pattern, on="company_id", how="left")
        .rename(columns={"capital_allocation_pattern": "capital_allocation_label"})
    )
