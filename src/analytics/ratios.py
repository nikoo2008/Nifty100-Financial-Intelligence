...
PERIODS = (3, 5, 10)
FINANCIAL_TICKERS = {"BAJFINANCE", "BAJAJFINSV", "HDFCBANK", "ICICIBANK", "INDUSINDBK", "KOTAKBANK", "SBILIFE", "SBIN"}


def _source_percent(value: Any) -> float | None:
    if value is None or pd.isna(value):
        return None
    match = pd.Series([str(value)]).str.extract(r"(-?\d+(?:\.\d+)?)%", expand=False).iloc[0]
...
def is_financial_company(company_id: Any, company_name: Any = None) -> bool:
    ...
    return text in FINANCIAL_TICKERS or any(word in name for word in ("bank", "finance", "financial", "life insurance"))
...
def calculate_ratios(pnl: pd.Series, balance: pd.Series | None = None, cashflow: pd.Series | None = None, company_name: Any = None) -> dict[str, Any]:
    ...
    sales, operating_profit, net_profit = _value(pnl, "sales"), _value(pnl, "operating_profit"), _value(pnl, "net_profit")
    equity = (_value(bs, "equity_capital") or 0) + (_value(bs, "reserves") or 0)
    borrowings, assets, interest = _value(bs, "borrowings"), _value(bs, "total_assets"), _value(pnl, "interest")
    ...
    if financial_carveout:
        ...
        ratios["carveout_note"] = "ROCE omitted: debt is operating funding for financial institutions."
    else:
...
def classify_capital_allocation(row: pd.Series) -> str:
    ...
    if pd.isna(row.get("operating_margin_pct")) or pd.isna(row.get("return_on_equity_pct")):
        return "insufficient_data"
    if row.get("net_margin_pct", 0) < 0 or row.get("interest_coverage", 0) is not None and row.get("interest_coverage", 0) < 1:
        return "distressed"
...
def build_financial_ratios(datasets: dict[str, pd.DataFrame], edge_log: Path = EDGE_LOG) -> pd.DataFrame:
    ...
    edge_lines: list[str] = []
    company_names = datasets.get("companies", pd.DataFrame()).set_index("id")["company_name"].to_dict() if "companies" in datasets else {}
    ...
    for _, p_row in pnl.iterrows():
        ...
        metrics = calculate_ratios(p_row, None if b_match.empty else b_match.iloc[-1], None if c_match.empty else c_match.iloc[-1], company_names.get(key[0]))
        metrics["dividend_payout"] = _value(p_row, "dividend_payout")
        source = analysis[analysis.company_id == key[0]]
        source_roe = None if source.empty else _source_percent(source.iloc[-1].get("roe"))
        metrics["source_roe_pct"] = source_roe
        metrics["roe_source_difference_pct"] = None if source_roe is None or metrics["return_on_equity_pct"] is None else metrics["return_on_equity_pct"] - source_roe
        metrics["roe_source_crosscheck"] = "matched" if metrics["roe_source_difference_pct"] is not None and abs(metrics["roe_source_difference_pct"]) <= 5 else ("review" if source_roe is not None else "unavailable")
        source_roce = None if source.empty else _source_percent(source.iloc[-1].get("roce"))
        metrics["source_roce_pct"] = source_roce
        metrics["roce_source_difference_pct"] = None if source_roce is None or metrics["return_on_capital_employed_pct"] is None else metrics["return_on_capital_employed_pct"] - source_roce
        metrics["roce_source_crosscheck"] = "matched" if metrics["roce_source_difference_pct"] is not None and abs(metrics["roce_source_difference_pct"]) <= 5 else ("review" if source_roce is not None else "unavailable")
        cagr = cagr_for_periods(pnl[pnl.company_id == key[0]], "sales", PERIODS)
        row = {"company_id": key[0], "year": None if pd.isna(key[1]) else int(key[1]), **metrics, **cagr}
        row["capital_allocation_pattern"] = None
        rows.append(row)
        if metrics.get("ratio_edge_case") or cagr.get("cagr_edge_case"):
            edge_lines.append(f"{key[0]} {key[1]} ratio={metrics.get('ratio_edge_case')} cagr={cagr.get('cagr_edge_case')}")
    edge_log.parent.mkdir(parents=True, exist_ok=True)
    edge_log.write_text("\n".join(edge_lines) + ("\n" if edge_lines else ""), encoding="utf-8")
    result = pd.DataFrame(rows)
    latest = result.sort_values(["company_id", "year"], na_position="first").drop_duplicates("company_id", keep="last")
    patterns = latest.set_index("company_id").apply(classify_capital_allocation, axis=1).to_dict()
